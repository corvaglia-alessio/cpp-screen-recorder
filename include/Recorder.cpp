...
void Recorder::open_audio_input(){
    ...
    const AVInputFormat* audio_in_fmt = av_find_input_format(audio_format.c_str());
    if(!audio_in_fmt)
    ...
    const AVCodec* audio_in_codec = avcodec_find_decoder(audio_in_stream->codecpar->codec_id);
    if(!audio_in_codec)
...
void Recorder::open_audio_output() {
    const AVCodec* audio_out_codec = avcodec_find_encoder(AUDIO_ENCODER);
    if(!audio_out_codec)
...
void Recorder::recorder_open_inputs(const string& video_input, int frames_per_second, string audio_input) {
    ...
    const AVInputFormat* video_in_fmt = av_find_input_format(video_format.c_str());
    if(!video_in_fmt)
    ...
    const AVCodec* video_in_codec = avcodec_find_decoder(video_in_codec_ctx->codec_id);
    if(!video_in_codec)
...
void Recorder::recorder_init_output(const string& path_name){
    ...
    const AVCodec* video_out_codec = avcodec_find_encoder(VIDEO_ENCODER);
    if(!video_out_codec)
